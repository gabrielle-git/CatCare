...
export default async function EditPetPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[760px] px-5 pb-8 pt-7 md:px-8 lg:py-10">
      ...
      {error ? (
        <div role="alert" className="mt-6 rounded-[20px] border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
          {error}
        </div>
      ) : null}
      <EditPetForm pet={pet} action={update} initialError={null} />
      <form action={archive} className="mt-4">
        <ConfirmButton
          message={`Arquivar ${pet.name}? O perfil some das listas, mas todo o histórico fica guardado.`}
          className="focus-ring flex w-full items-center justify-center gap-2 rounded-2xl border border-[var(--border)] bg-white px-5 py-3 text-sm font-bold text-[var(--muted)]"
        >
          <Archive size={17} /> Arquivar pet
        </ConfirmButton>
      </form>
    </div>
  );
