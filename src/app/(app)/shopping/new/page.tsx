...
async function loadForm() {
  if (!hasSupabaseEnv()) return { products: demoProducts, pets: demoPets, configured: false };
  const supabase = await createClient();
  const { data } = await supabase.auth.getUser();
  if (!data.user) return { products: [], pets: [], configured: true };
  const household = await ensureHousehold(supabase, data.user.id);
  const [{ products }, pets] = await Promise.all([listCommerce(supabase, household.id), listPets(supabase, household.id)]);
