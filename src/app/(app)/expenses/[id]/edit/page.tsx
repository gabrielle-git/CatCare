    ...
    <div className="mx-auto w-full max-w-[760px] px-5 pb-8 pt-7 md:px-8 lg:py-10">
      ...
      {expense.purchase_id && (
        <div className="mt-6 rounded-[20px] border border-[#cfe8d8] bg-[var(--mint-soft)] px-4 py-3 text-sm leading-relaxed">
          Este gasto veio de uma compra. Para alterar <strong>valor ou data</strong>, edite em{" "}
          <Link href={`/shopping/purchases/${expense.purchase_id}/edit`} className="font-bold text-[var(--success)] underline">Compras e avaliações</Link> — a alteração sincroniza aqui.
        </div>
      )}
      <form action={save} className="cat-card mt-6 space-y-5 p-5 md:p-7">
        ...
        <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold">Descrição<input required name="description" defaultValue={expense.description} className="field mt-2" /></label><label className="text-sm font-bold">Valor total (R$)<input required={!expense.purchase_id} name={expense.purchase_id ? undefined : "amount"} type="number" min="0" step="0.01" inputMode="decimal" defaultValue={amount} disabled={Boolean(expense.purchase_id)} className="field mt-2 disabled:opacity-60" /></label></div>
        <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold">Categoria<select required name="category" defaultValue={expense.category} className="field mt-2"><option value="veterinary">Veterinário</option><option value="food">Alimentação</option><option value="medication">Medicamentos</option><option value="hygiene">Higiene</option><option value="accessory">Acessórios</option><option value="transport">Transporte</option><option value="other">Outros</option></select></label><label className="text-sm font-bold">Data{expense.purchase_id ? <input required={false} type="date" defaultValue={date} disabled className="field mt-2 disabled:opacity-60" /> : <FactualDateInput required name="occurred_on" defaultValue={date} className="field mt-2" />}</label></div>
