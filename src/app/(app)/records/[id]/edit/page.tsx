...
import { RecordMutationForm } from "@/components/record-mutation-form";
...
export default async function EditRecordPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ source?: string; kind?: string; return_to?: string; error?: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[760px] px-5 pb-8 pt-7 md:px-8 lg:py-10">
      <Link href={returnTo} className="focus-ring inline-flex items-center gap-2 rounded-xl py-2 text-sm font-bold text-[var(--muted)]"><ArrowLeft size={17} /> Voltar</Link>
      ...
      <RecordMutationForm action={save} className="cat-card mt-6 p-5 md:p-7" initialError={query.error ?? null}>
