...
export default async function NewMemoryPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <div className="mx-auto w-full max-w-[820px] px-5 pb-8 pt-7 md:px-8 lg:py-10">
    ...
    {!configured && <div className="mt-6 rounded-[20px] bg-[var(--peach)] px-4 py-3 text-sm">O formulário está visível para comparação. <Link href="/login" className="font-bold underline">Crie sua conta ou faça login</Link> para salvar fotos privadas.</div>}
