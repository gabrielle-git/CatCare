...
export default async function AssistantPage() {
  ...
  const foodAgeNote = hasNeonatal ? " Essa recomendação não se aplica à alimentação neonatal; para os bebês, mantenha o protocolo orientado pelo veterinário." : hasKittens ? " Como há filhotes na família, confirme no rótulo se a fórmula é indicada para essa fase." : "";
  ...
  const answers = {
    vaccine: latestVaccine ? `A vacina mais recente registrada foi “${latestVaccine.title}” para ${names.get(latestVaccine.pet_id) ?? "um dos gatos"}, em ${formatDateTime(latestVaccine.occurred_at)}.` : "Ainda não encontrei nenhuma vacina registrada. Quando você lançar uma, ela aparecerá aqui com o gatinho e a data.",
    expenses: `Neste mês há ${monthExpenses.length} lançamentos, somando ${formatCurrency(monthTotal)}. Você pode abrir Gastos para ver a divisão por categoria e por gatinho.`,
    weight: currentWeights ? `Os pesos atuais registrados são: ${currentWeights}.${latestWeight ? ` A pesagem mais recente foi de ${names.get(latestWeight.pet_id) ?? "um gatinho"} em ${formatDateTime(latestWeight.occurred_at)}.` : ""}` : "Ainda não há pesos cadastrados. Registre uma pesagem para começar a acompanhar a evolução.",
    ...
    summary: `A família tem ${pets.length} ${pets.length === 1 ? "gatinho" : "gatinhos"}, ${reminders.length} lembretes pendentes e ${monthExpenses.length} gastos neste mês. Posso detalhar vacinas, pesos, agenda, gastos ou compras.`,
  };
