...
export function LoginForm({ configured, nextPath }: { configured: boolean; nextPath?: string }) {
  return (
    <form action={login} className="mt-6 space-y-4">
      ...
      <label className="block text-sm font-semibold">Senha<input required type="password" name="password" autoComplete="current-password" className="focus-ring mt-2 w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3 font-normal" /></label>
