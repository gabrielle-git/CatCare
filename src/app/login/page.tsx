...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string; next?: string }> }) {
  ...
  return (
    <AuthScreen title="Entrar">
      ...
      <p className="mt-5 text-sm text-[var(--muted)]">Ainda não tem conta? <Link href={cadastroHref} className="font-bold text-[var(--lavender-strong)] underline">Criar cadastro</Link></p>
    </AuthScreen>
  );
}
