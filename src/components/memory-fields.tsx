...
export function MemoryFields({
  ...
  defaultValues?: MemoryWithMediaUrl;
  disabled?: boolean;
    ...
    <div className="space-y-5">
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block text-sm font-bold">
          Tipo de lembrança
          <select disabled={disabled} name="type" defaultValue={defaultValues?.type ?? "milestone"} className="field mt-2">
            <option value="milestone">Primeira vez ou marco</option>
            <option value="diary">Momento do dia a dia</option>
            <option value="photo">Foto que merece ficar guardada</option>
          </select>
        </label>
        <label className="block text-sm font-bold">
          Quando aconteceu?
          <FactualDateTimeInput
            disabled={disabled}
            required
            name="occurred_at"
            defaultValue={defaultValues ? localDateTime(defaultValues.occurred_at) : ""}
      ...
      <label className="block text-sm font-bold">
        ...
        <input
          ...
          className="field mt-2"
          placeholder="Ex.: Abriu os olhinhos"
        />
      </label>
      <label className="block text-sm font-bold">
        Conte um pouquinho
        <textarea
          ...
          placeholder="O que aconteceu, quem estava junto e por que você quer lembrar desse momento?"
        />
      </label>
      <PetMultiSelect
        pets={pets.map((pet) => ({ id: pet.id, name: pet.name }))}
        defaultSelectedIds={defaultValues?.pet_ids}
        disabled={disabled}
